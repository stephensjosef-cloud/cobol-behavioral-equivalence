# Behavioral-Equivalence Findings â€” ACCTPROC / ACCTREC_A2

**Analyst:** Josef Stephens â€” Legacy Bridge
**Date:** 2026-06-10
...
> Deliverable type: *referenceable behavioral equivalence* â€” a documented,
...
month of interest: monthly rate = APR Ã· 12, interest = balance Ã— monthly rate,
...
round-half-away-from-zero on `COMPUTE â€¦ ROUNDED`, and the `ACCT-DEPOSIT`
...
- Cross-checked each item against the consolidated checklist trifecta:
  COMP-3 precision Â· copybook completeness Â· REDEFINES semantics.
...
> DATA QUALITY > PERFORMANCE. Don't pre-decide the verdict â€” record what the
> translation actually did.

### Finding 1 â€” CURR-BAL / COMP-3 precision
...
- **Verdict:** â˜‘ verified correct â˜ divergence
- **Business impact:** None â€” monetary precision preserved through accrual.
- **Severity:** n/a (verified correct)

### Finding 2 â€” APR-RATE scale (V9(5))
...
- **Verdict:** â˜‘ verified correct â˜ divergence
- **Business impact:** None â€” rate precision preserved; no scale truncation
  that would skew accrued interest.
- **Severity:** n/a (verified correct)

### Finding 3 â€” Rounding mode on COMPUTE ROUNDED
- **COBOL location:** `ACCTPROC.cbl:78` and `:80`, para 2100-ACCRUE-INTEREST
- **What the AI produced:** Both `COMPUTE â€¦ ROUNDED` use
...
- **Correct behavior:** RoundingMode.HALF_UP (round half away from zero) â€”
  the IBM Enterprise COBOL default. NOT HALF_EVEN.
- **Verdict:** â˜‘ verified correct â˜ divergence
- **Business impact:** None â€” interest pennies match the legacy job. This was
  the primary checkpoint; HALF_UP held across both COMPUTEs (no silent drop to
  banker's rounding).
- **Severity:** n/a (verified correct)

### Finding 4 â€” OVERDRAFT-DAYS binary COMP (contrast case)
- **COBOL location:** `ACCTREC_A2.cpy:19`, OVERDRAFT-DAYS PIC S9(4) COMP
- **What the AI produced:** `int overdraftDays` (`AcctProc.java:34`) â€” correctly
...
- **Correct behavior:** int/long â€” should NOT be widened to BigDecimal
- **Verdict:** â˜‘ verified correct â˜ divergence (with latent sign-decode note)
...
### Finding 5 â€” REDEFINES (DEPOSIT-DETAIL / LOAN-DETAIL)
...
- **What the AI produced:** A single shared `byte[18] acctDetail`; deposit and
  ...
  string. True discriminated union â€” NOT independent parallel fields.
- **Correct behavior:** shared-memory semantics gated by ACCT-TYPE-CODE
  (discriminated union / sealed type) â€” NOT two independent field sets
- **Verdict:** â˜‘ verified correct â˜ divergence
- **Business impact:** None â€” overlay semantics preserved; a record cannot be
  read as both arms simultaneously.
- **Severity:** n/a (verified correct)

### Finding 6 â€” MOVE CURR-BAL â†’ WS-BAL-ED edited display (PIC -(9)9.99)
- **COBOL location:** `ACCTPROC.cbl:87` (MOVE) / `:41` (WS-BAL-ED PIC -(9)9.99)
- **What the AI produced (initial):** `editBalance()` returned
  `sign + abs().toPlainString()` â€” correct 2-decimal value but **variable width,
...
- **Verdict:** â˜ verified correct â˜‘ divergence (**fixed + re-verified**)
- **Fix applied:** `editBalance()` rewritten to right-justify a floating-sign
  ...
  `0.00`, `0.50`, `987654321.99` â€” all emit 13-char output with correct sign
  placement; full translation recompiles clean.
- **Business impact (pre-fix):** `ACCTRPT.DAT` column alignment drifts and the
  `BAL=` field is no longer fixed-width â€” any downstream fixed-column parser or
  byte-for-byte report diff against the legacy output would mismatch despite the
  value being numerically correct.
- **Severity:** DATA QUALITY (report fidelity) â€” resolved.

### Finding 7 â€” (open risk) Record encoding / on-disk layout unverified
...
- **Verdict:** â˜ verified correct â˜‘ divergence â€” **unresolved / blocking**
...
## Summary
**4 verified correct** (Findings 1, 2, 3, 5) â€” the classic AI-translation traps
...
validated against a real `ACCTREC.DAT` sample â€” that test gates trust in every
